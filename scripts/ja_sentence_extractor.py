...
def is_valid_sentence(tokens):
    ...
    has_proper_end = (
        len(tokens[-1].part_of_speech()) > 1 and "句点" in tokens[-1].part_of_speech()[1]
    )
            ...
            for chunk in chunks:
                ...
                try:
                    ...
                    for word in tokens:
                        ...
                        is_predicate = len(pos) > 0 and (
                            pos[0] == "動詞" or pos[0] == "形容詞" or pos[0] == "助動詞"
                        )
                        if is_predicate:
                            has_predicate = True
                        ...
                        if len(pos) > 1 and "句点" in pos[1] and not in_quote:  # 。
                            ...
                            EMOJI_REGEX.match(surface) and prev_was_predicate and not in_quote
                        ):  # Emoji after predicate
                            should_split = True
